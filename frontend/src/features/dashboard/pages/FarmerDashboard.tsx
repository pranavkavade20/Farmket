    ...
    <div className="mx-auto max-w-[1400px] w-full pb-10">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-12 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-base font-bold text-gray-500 dark:text-gray-400">Here's what's happening with your farm store today.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        <StatCard title="Revenue" value={fmt(stats?.total_revenue ?? 0)} description="From delivered orders" icon={<TrendingUp className="h-6 w-6 text-white" />} color="bg-gradient-to-br from-blue-400 to-blue-600" delay={0.05} loading={statsLoading} />
        <StatCard title="Products Listed" value={String(stats?.total_products ?? 0)} description="Active listings" icon={<Package className="h-6 w-6 text-white" />} color="bg-gradient-to-br from-purple-400 to-purple-600" delay={0.1} loading={statsLoading} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="rounded-[2.5rem] bg-white dark:bg-[#111] border border-gray-100 dark:border-gray-800 shadow-sm p-10 relative overflow-hidden">
          ...
          <div className="mt-10 space-y-4 relative z-10">
            ...
            <Link to="/dashboard/analytics" className="block"><button className="w-full h-14 rounded-full bg-gray-100 text-gray-900 dark:bg-white/10 dark:text-white font-black text-sm flex items-center justify-center gap-2 hover:bg-gray-200 dark:hover:bg-white/20 transition-colors backdrop-blur-md"><TrendingUp className="h-4 w-4" /> View Analytics</button></Link>
          </div>
        </motion.div>
      </div>
    </div>
