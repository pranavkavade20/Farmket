...
const FlowNode = ({ 
  ...
  // Activate this node when scroll reaches its portion
  ...
  const pathProgress = useTransform(
    progress,
    [threshold, Math.min(1, threshold + 1 / totalNodes)],
    [0, 1]
  );
  ...
  return (
    <div className="relative flex flex-col items-center w-full max-w-[200px]">
      <motion.div 
        style={{ opacity, scale }}
        className="w-24 h-24 rounded-2xl bg-surface border-2 border-border-strong flex flex-col items-center justify-center shadow-lg z-10 relative overflow-hidden"
      >
      ...
      <motion.div style={{ opacity }} className="mt-6 text-center">
        <h4 className="font-bold text-lg">{title}</h4>
        <p className="text-sm text-foreground-secondary">{subtitle}</p>
      </motion.div>

      {!isLast && (
        <div className="absolute top-12 left-24 w-[calc(100%-48px)] md:w-[calc(100vw/5)] h-0.5 bg-border-subtle -z-10 hidden md:block">
          <motion.div 
      ...
      {!isLast && (
        <div className="w-0.5 h-16 bg-border-subtle md:hidden my-4 relative">
          <motion.div 
...
export const MarketplaceFlow = () => {
  ...
  return (
    <section className="bg-background py-32 relative">
      ...
      <div 
        ref={containerRef} 
        className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between px-6 md:px-12 relative pb-32"
      >
